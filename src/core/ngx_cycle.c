...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    // Nginx 处理配置文件时需要特殊处理的在命令行携带的参数，  一般是 -g 选项所携带的参数
    ...
    if (old_cycle->open_files.part.nelts) {
        ...
        // 若 old_cycle->open_files 链表为空，则默认设置下面创建的 open_files 链表  元素个数为 20
        n = 20;
    }
    ...
    if (old_cycle->shared_memory.part.nelts) { // 统计个数
        ...
        // 若 old_cycle->shared_memory 链表文件，则默认下面创建的 shared_memory 链表  元素个数为 1
        n = 1;
    }
    //共享内存的初始化  创建 shared_memory 链表，元素类型是 ngx_shm_zone_t 结构体，每个元素表示 一块共享内存
    ...
    //连接监听的数组,具体结构附录图  创建 listening 数组，每个数组元素存储着 ngx_listening_t 成员，表示监听端口及相关的参数
    ...
    // 初始化 reusable_connections_queue 双向链表容器，元素类型是 ngx_connection_t 结构体，表示可重复使用连接队列
    ...
    for (i = 0; cycle->modules[i]; i++) {
        ...
        if (module->create_conf) {
            ...
            // 将返回的配置信息结构体保存到该核心模块在 cycle->conf_ctx 数组 对应下标处
    ...
    // 开始解析配置文件中的所有配置项, conf_file 保存着配置文件  的绝对路径
    ...
    // 若当前系统不支持原子操作时则检测用于文件锁的文件是否打开， 若失败，则返回 error
    ...
    for (i = 0; /* void */ ; i++) {
        ...
        // 每个模块的共享内存都有自己特有的属性，因此调用各自的 共享内存初始化函数
    ...
    if (old_cycle->listening.nelts) {
        ...
        // listening: 动态数组，每个数组元素存储着ngx_listening_t成员， 表示监听端口及相关的参数
        ls = cycle->listening.elts;
        for (i = 0; i < cycle->listening.nelts; i++) {
            // 标志位，为1则表示在当前监听句柄有效，且执行ngx_init_cycle时 不关闭监听端口，为0时则正常关闭
            ls[i].open = 1;
#if (NGX_HAVE_DEFERRED_ACCEPT && defined SO_ACCEPTFILTER)
